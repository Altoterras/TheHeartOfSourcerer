...
JsKeywordSet::JsKeywordSet()
{
	...
	listSign.addTail(new EsKeyword("[",				TKN_LB,			OP_NOP));
	listSign.addTail(new EsKeyword("]",				TKN_RB,			OP_NOP));
	listSign.addTail(new EsKeyword("{",				TKN_LC,			OP_NOP));
	listSign.addTail(new EsKeyword("}",				TKN_RC,			OP_NOP));
	listSign.addTail(new EsKeyword("(",				TKN_LP,			OP_NOP));
	listSign.addTail(new EsKeyword(")",				TKN_RP,			OP_NOP));
	listSign.addTail(new EsKeyword(",",				TKN_COMMA,		OP_NOP));
	listSign.addTail(new EsKeyword("?",				TKN_HOOK,		OP_NOP));
	listSign.addTail(new EsKeyword(".",				TKN_DOT,		OP_NOP));
	listSign.addTail(new EsKeyword(":",				TKN_COLON,		OP_NOP));
	listSign.addTail(new EsKeyword("||",			TKN_OR,			OP_NOP));
	listSign.addTail(new EsKeyword("|=",			TKN_ASSIGN,		OP_BITOR));
	listSign.addTail(new EsKeyword("|",				TKN_BITOR,		OP_NOP));
	listSign.addTail(new EsKeyword("^=",			TKN_ASSIGN,		OP_BITXOR));
	listSign.addTail(new EsKeyword("^",				TKN_BITXOR,		OP_NOP));
	listSign.addTail(new EsKeyword("&&",			TKN_AND,		OP_NOP));
	listSign.addTail(new EsKeyword("&=",			TKN_ASSIGN,		OP_BITAND));
	listSign.addTail(new EsKeyword("&",				TKN_BITAND,		OP_NOP));
	listSign.addTail(new EsKeyword("===",			TKN_EQOP,		OP_STRICTEQ));
	listSign.addTail(new EsKeyword("==",			TKN_EQOP,		OP_EQ));
	listSign.addTail(new EsKeyword("=",				TKN_ASSIGN,		OP_NOP));
	listSign.addTail(new EsKeyword("!==",			TKN_EQOP,		OP_STRICTNE));
	listSign.addTail(new EsKeyword("!=",			TKN_EQOP,		OP_NE));
	listSign.addTail(new EsKeyword("!",				TKN_UNARYOP,	OP_NOT));
	listSign.addTail(new EsKeyword("<!--",			TKN_NULL,		OP_NULL));		// 要検討修正
	listSign.addTail(new EsKeyword("<<=",			TKN_ASSIGN,		OP_LSFT));
	listSign.addTail(new EsKeyword("<<",			TKN_SFTOP,		OP_LSFT));
	listSign.addTail(new EsKeyword("<=",			TKN_INQOP,		OP_LE));
	listSign.addTail(new EsKeyword("<",				TKN_INQOP,		OP_LT));
	listSign.addTail(new EsKeyword(">>>=",			TKN_ASSIGN,		OP_URSFT));
	listSign.addTail(new EsKeyword(">>>",			TKN_SFTOP,		OP_URSFT));
	listSign.addTail(new EsKeyword(">>=",			TKN_ASSIGN,		OP_RSFT));
	listSign.addTail(new EsKeyword(">>",			TKN_SFTOP,		OP_RSFT));
	listSign.addTail(new EsKeyword(">=",			TKN_INQOP,		OP_GE));
	listSign.addTail(new EsKeyword(">",				TKN_INQOP,		OP_GT));
	listSign.addTail(new EsKeyword("*=",			TKN_ASSIGN,		OP_MUL));
	listSign.addTail(new EsKeyword("*",				TKN_STAR,		OP_MUL));
	listSign.addTail(new EsKeyword("//",			TKN_NULL,		OP_NULL));		// 要検討修正
	listSign.addTail(new EsKeyword("/*",			TKN_NULL,		OP_NULL));		// 要検討修正
	listSign.addTail(new EsKeyword("/=",			TKN_ASSIGN,		OP_DIV));
	listSign.addTail(new EsKeyword("/",				TKN_DIVOP,		OP_DIV));
	listSign.addTail(new EsKeyword("%=",			TKN_ASSIGN,		OP_MOD));
	listSign.addTail(new EsKeyword("%",				TKN_DIVOP,		OP_MOD));
	listSign.addTail(new EsKeyword("~",				TKN_UNARYOP,	OP_BITNOT));
	listSign.addTail(new EsKeyword("+=",			TKN_ASSIGN,		OP_ADD));
	listSign.addTail(new EsKeyword("++",			TKN_INC,		OP_NOP));
	listSign.addTail(new EsKeyword("+",				TKN_PLUS,		OP_POS));
	listSign.addTail(new EsKeyword("-->",			TKN_NULL,		OP_NULL));		// 要検討修正
	listSign.addTail(new EsKeyword("-=",			TKN_ASSIGN,		OP_SUB));
	listSign.addTail(new EsKeyword("--",			TKN_DEC,		OP_NOP));
	listSign.addTail(new EsKeyword("-",				TKN_MINUS,		OP_NEG));
	// 配列化
	...
	_kwarrStringQuote[2] = new EsKeyword("\"", TKN_NULL, OP_NULL);	// 他言語用に互換キーワードを用意しておく
	_kwarrStringQuote[3] = new EsKeyword("'", TKN_NULL, OP_NULL);	// 他言語用に互換キーワードを用意しておく
