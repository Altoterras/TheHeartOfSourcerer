...
#ifndef MULTIPLE_THREADS
	if (dtoa_result) {
		freedtoa_r(dtoa_result);//r//ↁE/freedtoa(dtoa_result);
		dtoa_result = 0;
		}
#endif
...
static u32 quorem2(Bigint *b, s32 k)
{
    ULong mask;
    ULong result;
    ULong *bx, *bxe;
...
char* _dtoa_radixed(double dval, int radix)
{
    U d;
    ...
    if (buffer) {
        ...
        if (dval(df) != 0.0) {
            ...
            *p++ = '.';
            ...
            s2 = -(s32)(word0(d) >> Exp_shift1 & Exp_mask>>Exp_shift1);
#ifndef Sudden_Underflow
            if (!s2)
                s2 = -1;
#endif
            ...
            do {
                ...
                digit = quorem2(b, s2);
                if (mlo == mhi) {
                    mlo = mhi = multadd(mlo, radix, 0);
                    if (!mhi)
                        goto nomem2;
                }
                else {
                    mlo = multadd(mlo, radix, 0);
                    if (!mlo)
                        goto nomem2;
                    mhi = multadd(mhi, radix, 0);
                    if (!mhi)
                        goto nomem2;
                }
