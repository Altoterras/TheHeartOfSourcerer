...
// Self
#include "QsEmp_Fixed_ActualCancel.h"
...
/*---------------------------------------------------------------------*//**
	開始
**//*---------------------------------------------------------------------*/
void QsEmp_Fixed_ActualCancel::begin(u32 param)
{
	QsEmp::begin(param);
...
/*---------------------------------------------------------------------*//**
	終了
**//*---------------------------------------------------------------------*/
void QsEmp_Fixed_ActualCancel::end()
{
	// スプライト削除
	if(_sprite != 0L)
	{
...
/*---------------------------------------------------------------------*//**
	コンストラクタ
**//*---------------------------------------------------------------------*/
QsEmp_Fixed_ActualCancel::QsEmp_Fixed_ActualCancel()
	: QsEmp(QSEMPTYPE_ACTUAL_CANCEL_QUEST)
	, _sprite(0L)
{
}
...
/*---------------------------------------------------------------------*//**
	フレーム処理実装
**//*---------------------------------------------------------------------*/
void QsEmp_Fixed_ActualCancel::execImplement(ExecRes* res, const ExecCtx* ec, f32 frameBegun)
{
	...
	case STEP_WAIT_BEGIN:		// 継続監視
		if(!getOwner()->checkBegunOtherMatter(this, true))	// 自身以外のイベントが起動していない
		{
			...
			// スプライトを読み込む
			///_sprite->create(FILEID_CONV_LANG(QUESTEVENT_SPRITE_QUESTFINISH_EN_PNG), 0, true);

			// SE 再生
			Game::getGame()->getSoundManager()->playUiSe(GameSoundDef::SE_QUEST_CANCEL, false);
		}
		break;
...
/*---------------------------------------------------------------------*//**
	2D 描画
**//*---------------------------------------------------------------------*/
void QsEmp_Fixed_ActualCancel::draw(const RenderCtx* rc)
{
	switch(getStep())
	{
	case STEP_WAIT_EFFECT:
...
/*---------------------------------------------------------------------*//**
	クエストクリア
**//*---------------------------------------------------------------------*/
void QsEmp_Fixed_ActualCancel::cancelQuest()
{
	...
	qstmng->cancelQuest(qsta->getDef()->getQuestId());
}
