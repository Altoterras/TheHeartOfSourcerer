...
/*---------------------------------------------------------------------*//**
	XML 要素から読み込む
**//*---------------------------------------------------------------------*/
bool ShadowCtrl::analyze(ActMatter* owner, const XmlNode* node, const EventMatterCreateParam* emcp)
{
	...
	// 動作設定解析
	if(!ShadowCtrlMatter::analyzeSettingFromXml(&_setting, node))
	{
		return false;
	}
...
/*---------------------------------------------------------------------*//**
	動作開始
**//*---------------------------------------------------------------------*/
void ShadowCtrl::play(ActMatter* owner)
{
	EventData* evdat = owner->getEventSystemRefer()->getEventData(); ASSERT(evdat != 0L);

	// スプライト表示
	_evid = evdat->addChildMatter(owner->getEvId(), "SHADOWCTRL"); ASSERT(_evid != 0);
	ShadowCtrlMatter* matter = (ShadowCtrlMatter*)evdat->getMatterFromEvid(_evid); ASSERT(matter != 0L);
...
/*---------------------------------------------------------------------*//**
	動作停止
**//*---------------------------------------------------------------------*/
void ShadowCtrl::stop(ActMatter* owner)
{
	EventData* evdat = owner->getEventSystemRefer()->getEventData(); ASSERT(evdat != 0L);
