...
// External
#include "../../sourcerer/sg/TbfDefineEx.h"
...
/*---------------------------------------------------------------------*//**
	パーティクルモデルを読み込む
**//*---------------------------------------------------------------------*/
Model* DimzParticleSys::loadModel(s32 pmdlid)
{
	Model* model = 0L;
	...
	else
	{
		...
		{
			TRACE("{DimzParticleSys::loadModel} no file: fileid=%d\n", fileid);
			return 0L;
		}

		if(!ModelLoader::loadModelFromTbf(DimzBody::getBody()->getTfw(), fileid, TbfDefineEx::MEXF_PTCL, ModelLoader::LFLAG_LOAD_BILLBOARD, (Model**)&model))
		{
			ASSERT(false);
			return 0L;
		}
